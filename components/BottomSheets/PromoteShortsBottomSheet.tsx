...
export default function PromoteShortsBottomSheet({ setOpenSheet }: { setOpenSheet: (fn: () => void) => void }) {
  ...
  const handleSheetChanges = useCallback((index: number) => {
    if (index === -1) {
      setShortsUrl(''); // Clear the URL when the sheet is closed
    }
  }, []);

  const handleBoostNow = () => {
    // Add your boost logic here, e.g., validate URL, make API call
    console.log('Boosting shorts:', shortsUrl);
  ...
  return (
    <Sheet ref={bottomSheetModalRef} snapPoints={snapPoints}>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <Text variant={'heading'} className='mb-4 text-center'>Promote Shorts</Text>
        <Text variant={'subhead'} className="mb-2 text-center">Increase views on your shorts using coins</Text>
        ...
        <TextInput
          ...
          value={shortsUrl}
          onChange={(ev) => setShortsUrl(ev.nativeEvent.text)}
          mode='outlined'
          className="mb-4" />
        {/* <TextInput
        ...
        <TouchableOpacity
          onPress={() => {
            ...
            bottomSheetModalRef.current?.close();
            handleBoostNow();
            router.push('/promoteshortsplans');
            handleSheetChanges(-1);
          }}
